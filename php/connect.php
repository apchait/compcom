...
function connectToDb(){
	...
	if ($onIschool){
		$link = mysql_connect('', 'acopio', 'acopio11235') or die(mysql_errno() . mysql_error()); 
		mysql_select_db('acopio') or die ('Could not select: ' . mysql_error() . mysql_error());
	}
	else{
		$link = mysql_connect('localhost', 'root', 'root') or die(mysql_errno() . mysql_error()); 
		mysql_select_db('acopio') or die ('Could not select: ' . mysql_error() . mysql_error());
	}
	return $link;
}
